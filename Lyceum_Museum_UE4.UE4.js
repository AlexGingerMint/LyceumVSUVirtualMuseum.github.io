...
console.log("Emscripten version: D:/UE4_WebBuild/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-3.1.50/upstream/emscripten");
console.log("Emscripten configuration: D:/UE4_WebBuild/ue-4.27-html5-es3/Engine/Platforms/HTML5/Build/emsdk/emsdk-3.1.50/.emscripten");
...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_Lyceum_Museum_UE4', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202401050613, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = sessionStorageCommandLine != null ? [sessionStorageCommandLine] : ['../../../Lyceum_Museum_UE4/Lyceum_Museum_UE4.uproject','-stdout',];
...
function resizeCanvas(aboutToEnterFullscreen) {
	...
	var cssHeight = Math.max(minimumCanvasHeightCssPixels, mainAreaRect.bottom - mainAreaRect.top, window.innerHeight * minimumCanvasHeightFractionOfBrowserWindowHeight - buttonAreaRect.height);
...
if (!enableReadFromIndexedDB) console.warn('Running with IndexedDB access disabled.');
else if (!enableWriteToIndexedDB) console.warn('Running in read-only IndexedDB access mode.');
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Lyceum_Museum_UE4.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('Lyceum_Museum_UE4.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('Lyceum_Museum_UE4.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('Lyceum_Museum_UE4.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('Lyceum_Museum_UE4.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
